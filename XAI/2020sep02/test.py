...
def file_write(string, file_name):
    f = open(file_name, "w")
    f.write(string)
    f.close


def cycle_prediction(cycle_num, question, text, file_name, plot):
    ...
    file_write("第一次预测的结果是" + first_answer, file_name)
...
for i in list(range(300, 400)):
    text = datasets['train'][i]['context']
    question = datasets['train'][i]['question']
    answers = datasets['train'][i]['answers']
    cycle_prediction(20, question, text, file_name, plot=0)
    file_write(text, file_name)
    file_write(question, file_name)
    print("standed answer is : ", answers)
    print(text)
    print(question)
