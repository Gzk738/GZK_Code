...
wrong_ids = []
import json
path_example = r'D:\software\github\GZK_Code\XAI\2021oct07\hotpot_dev_distractor_v1.json'
with open(path_example, 'r') as load_f:
    datasets = json.load(load_f)



for i in datasets[:100]:
    text = ''
    for j in i['context']:
        for k in j[1]:
            text += k + ', '
    ...
    question = i['question']
    answers = i['answer']
    f1, acc_s, acc_e, sun = muti_pre(10, question, text, answers, 0.9, 0.7)
    ...
    if len(f1) != 1:
        if max(sun) > 0.9:
            print("sun:第", i, "个的f1 score变化是", f1_change, "概率的变化是", pro_change0, pro_change1, pro_change2)
        if max(acc_s) > 0.9:
            print("acc_S:第", i, "个的f1 score变化是", f1_change, "概率的变化是", pro_change0, pro_change1, pro_change2)
        if max(acc_e) > 0.9:
            print("acc_e:第", i, "个的f1 score变化是", f1_change, "概率的变化是", pro_change0, pro_change1, pro_change2)
    if len(collections.Counter(f1)) == 1 and len(f1) != 1:
        print("F1 score 没变化的是id是", i, "概率变化是", pro_change0, pro_change1, pro_change2, f1, sun)
    print(i, "个完成了")
    g_f1.append(f1)
    g_accs.append(acc_s)
    g_acce.append(acc_e)
